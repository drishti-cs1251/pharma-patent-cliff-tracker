...
export default function SearchBar() {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    if (!query.trim()) { setError('Please enter a drug name'); return; }
    setShowSuggestions(false);
    performSearch(query);
  };
  ...
  const performSearch = async (searchQuery) => {
    ...
    try {
      ...
      if (results.length === 0) setError(`No results found for "${searchQuery}"`);
    } catch (err) {
  ...
  const handleDrugSelect = async (drug) => {
    ...
    try {
      ...
      if (altData) {
        ...
        if (altData.alternatives.length === 0) setError(`No alternatives found for ${drug.name}`);
      } else {
              ...
              <ul className="suggestions-dropdown">
                {suggestions.map((name) => (
                  <li
                    ...
                    onMouseDown={() => { setQuery(name); setShowSuggestions(false); performSearch(name); }}
                  >
